...
@st.cache_data
def load_missing_persons_data(filepath):
    """
    Loads and cleans the Missing Persons dataset.
    """
    try:
        # This line correctly uses the 'filepath' variable
        df = pd.read_csv(filepath, na_values=[""], low_memory=False)
        
        # Drop columns that are entirely null (often artifacts of CSV export)
        df = df.dropna(how='all', axis=1)
        
        # Identify gender/age columns
        male_cols = [col for col in df.columns if 'male_' in col]
        female_cols = [col for col in df.columns if 'female_' in col]
        trans_cols = [col for col in df.columns if 'trangender_' in col or 'transgender_' in col]
        ...
        df['total_missing_male'] = df[male_cols].sum(axis=1)
        df['total_missing_female'] = df[female_cols].sum(axis=1)
        df['total_missing_trans'] = df[trans_cols].sum(axis=1)
        ...
        st.error(f"An error occurred while loading the missing persons data: {e}")
        return None, None

@st.cache_data
def load_juvenile_crime_data(filepath):
    """
    ...
    try:
        ...
        # Filter out some potential summary rows
        ...
        st.error(f"An error occurred while loading the juvenile crime data: {e}")
        return None, None

# --- Load Data ---
# *** THIS IS THE FIX ***
# We define the full paths to your files here, in quotes.
missing_persons_path = '/Users/padmanabhjagdishwanikar/Downloads/crime-analytics-project/districtwise-missing-persons-2017-2022.csv'
juvenile_crime_path = '/Users/padmanabhjagdishwanikar/Downloads/crime-analytics-project/districtwise-ipc-crime-by-juveniles-2017-onwards.csv'
...
@st.cache_data
def get_merged_data(_missing_df, _juvenile_df):
    """
    ...
    missing_agg = _missing_df.groupby(['year', 'state_name', 'district_name'])['total_missing'].sum().reset_index()
    juvenile_agg = _juvenile_df.groupby(['year', 'state_name', 'district_name'])['total_ipc_crimes'].sum().reset_index()
...
if missing_df_raw is None or juvenile_df_raw is None:
    ...
    page = st.sidebar.radio(
        "Select a Page",
        ("🏠 Home", "📊 Exploratory Data Analysis (EDA)", "📍 District Deep Dive", "🧍 Missing Persons Analysis", "⚖️ Juvenile Crime Analysis", "🔬 Advanced Analysis (Demo)")
    )
    ...
    all_states = sorted(list(pd.concat([missing_df_raw['state_name'], juvenile_df_raw['state_name']]).unique()))
    all_years = sorted(list(pd.concat([missing_df_raw['year'], juvenile_df_raw['year']]).unique()))
    ...
    def show_home_page():
        ...
        **Data Filters Applied:**
        ...
        time_df = pd.merge(missing_time, juvenile_time, on='year', how='outer').fillna(0)
    ...
    def show_district_deep_dive_page():
        """
        New page for detailed analysis of a single selected district.
        ...
        try:
            all_states_list = sorted(list(pd.concat([missing_df_raw['state_name'], juvenile_df_raw['state_name']]).unique()))
            ...
            if selected_state_dd:
                available_districts = sorted(list(pd.concat([
        ...
        with col1:
            ...
            if not missing_district_df.empty:
                ...
                age_df['Gender'] = age_df['Category'].apply(lambda x: 'Male' if 'male_' in x else ('Female' if 'female_' in x else 'Transgender'))
                age_df['Age Group'] = age_df['Category'].str.replace('male_', '').str.replace('female_', '').str.replace('trangender_', '').str.replace('transgender_', '').str.replace('_', ' ').str.title()
        ...
        with col2:
            ...
            if not juvenile_district_df.empty:
                ...
                crime_data.columns = ['Crime Type', 'Total Cases']
                
                if not crime_data.empty and crime_data['Total Cases'].sum() > 0:
                    fig_crime = px.bar(
                        crime_data,
                        x='Total Cases',
                        y='Crime Type',
                    ...
                    fig_crime.update_layout(yaxis={'categoryorder':'total ascending'})
    ...
    def show_missing_persons_page():
        ...
        available_districts = sorted(list(missing_df['district_name'].unique()))
        ...
        with col2:
            st.subheader(f"Top 10 Districts (Filtered)")
    ...
    def show_juvenile_crime_page():
        ...
        # District Filter (specific to this page)
        available_districts = sorted(list(juvenile_df['district_name'].unique()))
        selected_district = st.selectbox(
            "Select District (Optional)",
            ["All"] + available_districts,
            index=0
    ...
    def show_advanced_analysis_page():
        ...
        X = cluster_data[['total_missing', 'total_ipc_crimes']]
        ...
        X_reg = cluster_data[['total_ipc_crimes']]
        y_reg = cluster_data['total_missing']
        ...
        fig_reg.update_traces(marker=dict(color='#007bff', opacity=0.7))
    ...
    if __name__ == "__main__":
        if missing_df_raw is not None and juvenile_df_raw is not None:
            ...
            elif page == "⚖️ Juvenile Crime Analysis":
                show_juvenile_crime_page()
            elif page == "🔬 Advanced Analysis (Demo)":
                show_advanced_analysis_page()
        else:
            st.title("Error")
            st.error("Data could not be loaded. Please ensure the file paths at the top of the script are correct.")
